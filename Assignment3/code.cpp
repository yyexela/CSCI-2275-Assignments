...
class WordAnalysis{
	...
	int getWordCount(string word){
		for(int i = 0; i < wordsCount; i++){
			if(words[i].getWord() == word) return words[i].getCount();
		}	
	}

	//print all words containing string wrd as its initial substring
	void printSub(string wrd){
		//counter for the amount of times the word is contained in the start of a word
		int total = 0;
		for(int i = 0; i < wordsCount; i++){
			//loop through the array and add to the counter if the word is encountered in the beginning of a word
			if(words[i].getWord().size() >= wrd.size() && words[i].getWord().substr(0, wrd.size()) == wrd){
				total += getWordCount(words[i].getWord());
			}
		}
		//print the word
		cout<<wrd<<" - "<<total<<endl;
	...
	//search for word in command line arguments
	//run printWord for every word delimeted by comma
	void searchCount(string arg){
		...
		if(end == string::npos){
			...
			printSub(arg.substr(0,arg.find(del)));
			//keep printing words so long as there are commas
			while(index != end){
				printSub(arg.substr(index+1,arg.find(del,index+1)-index-1));
				//update comma marker
				index = arg.find(del, index+1);
			}
			//print final word past last comma
			printSub(arg.substr(end+1));
		}
	}
